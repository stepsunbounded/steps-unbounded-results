...
theorem pair_cap_coset (K L : Subgroup P) (x y : P) (hm : Nat.card K ∈ lowerSizes)
  ...
  have hcap : (((cos K x) \ Hset) ∩ ((cos L y) \ Hset)).card ≤
      Nat.card ↥(K ⊓ L : Subgroup P) := by
...
theorem card_A5fin : A5fin.card = 60 := by
  ...
  have h2 : Fintype.card {x : P // Equiv.Perm.sign x = 1} = Fintype.card ↥A5 :=
    Fintype.card_congr
      (Equiv.subtypeEquivRight (fun x : P => Equiv.Perm.mem_alternatingGroup)).symm
...
theorem image_mul_eq_A5_of_card (K : Subgroup P) (hK : K ≤ A5)
  ...
  exact image_mul_eq_of_card (fun k hk l hl => mem_A5fin.mpr (A5.mul_mem (H_le_A5 hk) (hK hl)))
    hcard'
...
theorem no_order_six_subgroup_of_H (N : Subgroup P) (hN : N ≤ H) (hcard : Nat.card N = 6) :
    False := by
  classical
  ...
  have key : ∀ x ∈ Hset, x * x * x = 1 → x ≠ 1 → x ∈ Nf := by
    ...
    have hdisj : Disjoint Nf (Nf.image (fun n => x * n)) := by
      ...
      have hxy : x = y * n⁻¹ := by rw [← hny]; group
      exact hxN (by rw [hxy]; exact hNf_mul y hy n⁻¹ (hNf_inv n hn))
...
theorem lemma43_coset (K L : Subgroup P) (x y : P) (hK : K ≤ A5) (hx : x ∈ A5)
  ...
  · by_cases hT : Nat.card ↥(K ⊓ L) = 2
    · by_contra hgt
      ...
      obtain ⟨a, ha, b, hb, hab⟩ := (Finset.one_lt_card (s := ((cos K x) \ Hset) ∩ ((cos L y) \ Hset))).mp (by omega)
